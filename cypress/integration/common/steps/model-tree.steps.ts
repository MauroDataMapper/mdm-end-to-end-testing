...
When(/^I click on "([^"]*)" in the model tree$/, (label) => {
  cy.wrap<ModelTreeNodeSelection>({ label })
    .as('currentItem')
    .then(() => page.treeView.tree.ensureExpanded(['Development Folder'])
    .then(() => page.treeView.tree.getTreeNode(label).click()));
});

When(/^I click on "([^"]*)" with version "([^"]*)" in the model tree$/, (label, version) => {
  cy.wrap<ModelTreeNodeSelection>({ label, version })
    .as('currentItem')
    .then(() => page.treeView.tree.ensureExpanded(['Development Folder'])
    .then(() => page.treeView.tree.getTreeNode(label, version).click()));
});